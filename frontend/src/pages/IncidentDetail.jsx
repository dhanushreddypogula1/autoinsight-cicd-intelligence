...
import {
  ArrowLeft, AlertTriangle, GitBranch, Cpu,
  Calendar, FileText, Hash, Layers, Bug, Wrench,
} from 'lucide-react'
import { useFetch } from '../hooks/useFetch'
import { getIncidentById } from '../services/api'
import {
  Card, CardHeader, SeverityBadge, CategoryBadge,
  Skeleton, ErrorState,
} from '../components/ui'
  ...
  return (
    <div className="flex items-start gap-2.5 p-3 bg-surface-3 rounded-lg border border-border-dim">
      ...
      <div className="min-w-0">
        ...
        <p className={cx('text-sm text-slate-200 mt-0.5 truncate', mono ? 'font-mono' : 'font-medium')}>
          {value || '—'}
        </p>
      </div>
    </div>
...
function CountChip({ label, value, color }) {
  return (
    <div className={cx('flex flex-col items-center p-3 rounded-lg border', color)}>
      <span className="text-2xl font-display font-bold tabular-nums">{value ?? 0}</span>
      <span className="text-[10px] font-mono text-slate-500 mt-0.5">{label}</span>
    </div>
  )
}
...
function CodeBlock({ title, content, icon: Icon }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <pre className="bg-surface-0 border border-border-dim rounded-xl p-4 text-xs font-mono text-slate-300
                      overflow-x-auto whitespace-pre-wrap leading-relaxed max-h-64 overflow-y-auto">
        {content}
      </pre>
    </div>
  )
...
export default function IncidentDetail() {
  ...
  if (loading) return <div className="animate-fade-in"><SkeletonDetail /></div>
    ...
    <div className="space-y-5 animate-fade-in max-w-4xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h2 className="font-display font-bold text-white text-xl leading-snug">{inc.title}</h2>
          {inc.summary && (
            <p className="text-sm text-slate-400 mt-2 leading-relaxed">{inc.summary}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        <CountChip label="Errors"      value={inc.errorCount}      color="bg-red-500/5 border-red-500/20 text-red-400"     />
        <CountChip label="Warnings"    value={inc.warningCount}    color="bg-yellow-500/5 border-yellow-500/20 text-yellow-400" />
        <CountChip label="Exceptions"  value={inc.exceptionCount}  color="bg-orange-500/5 border-orange-500/20 text-orange-400" />
        <CountChip label="Stack Traces"value={inc.stackTraceCount} color="bg-purple-500/5 border-purple-500/20 text-purple-400" />
      </div>
          ...
          {inc.probableRootCause && (
            <div className="p-4 bg-surface-3 rounded-xl border border-border-dim">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-mono text-slate-500 uppercase tracking-wider">Probable Root Cause</p>
              </div>
              <p className="text-sm text-slate-300 leading-relaxed">{inc.probableRootCause}</p>
            </div>
          )}
          ...
          {inc.suggestedFix && (
            <div className="p-4 bg-surface-3 rounded-xl border border-accent-teal/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-mono text-slate-500 uppercase tracking-wider">Suggested Fix</p>
              </div>
              <p className="text-sm text-slate-300 leading-relaxed">{inc.suggestedFix}</p>
            </div>
          )}
        </div>
      </Card>
      ...
      {(inc.rawErrorLines || inc.rawExceptionLines) && (
        <Card>
          <CardHeader title="Raw Log Excerpts" subtitle="Extracted error and exception lines" />
          <div className="space-y-5">
