...
return ( <div className="overflow-x-auto"> <table className="w-full text-sm"> <thead> <tr className="border-b border-border-dim">
{['ID', 'Title', 'Severity', 'Category', 'Pipeline', 'Errors', 'Date', 'Actions'].map(h => ( <th
             key={h}
             className="text-left text-[10px] font-mono text-slate-600 uppercase tracking-widest pb-2 px-3 first:pl-0 last:pr-0 whitespace-nowrap"
           >
{h} </th>
))} </tr> </thead>
    ...
    <tbody>
      {(compact ? incidents.slice(0, 5) : incidents).map((inc) => (
        <tr
          ...
          className="border-b border-border-dim/50 hover:bg-surface-3/50 cursor-pointer transition-colors group"
        >
